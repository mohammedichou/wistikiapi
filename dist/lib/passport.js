...
var getSettings = function getSettings(settings) {
	...
	var result = typeof settings === 'function' ? settings(defaults) : _.extend(defaults, settings);
...
module.exports = function (configuration) {
	...
	return function () {
		...
		app.use(settings.cookieParser(settings.secret)).use(settings.session).use(settings.passport.initialize()).use(settings.passport.session()).use(function (req, res, next) {
